    ...
    if (assessment.type === 'vulnerability' || assessment.type === 'code') {
      ...
      if (vulnerabilities.length === 0) {
        ...
        vulnerabilities.forEach((vuln, index) => {
          ...
          y += 7;
          const splitMitigation = doc.splitTextToSize(`Mitigation: ${vuln.mitigation || "N/A"}`, pageWidth - margin - 10);
          doc.text(splitMitigation, margin + 5, y);
          y += splitMitigation.length * 7 + 5;
        });
      }
    } else {
      doc.text("No detailed results available for this assessment type.", margin, y);
      y += 8;
    }
    ...
    <DashboardContainer>
      ...
      <StyledAppBar position="static" color="default">
        <Tabs value={tabValue} onChange={handleTabChange} variant="fullWidth" indicatorColor="primary" textColor="primary">
          ...
          <Tab label="Data Assessment" id="assessment-tab-3" aria-controls="assessment-tabpanel-3" />
        </Tabs>
      </StyledAppBar>
      ...
      <Dialog open={openDetailsModal} onClose={handleCloseDetailsModal} fullWidth maxWidth="md" sx={{ backdropFilter: 'blur(4px)' }}>
        <DialogTitle sx={{ bgcolor: '#00bcd4', color: '#fff' }}>Assessment Details</DialogTitle>
        <DialogContent dividers sx={{ bgcolor: 'rgba(255,255,255,0.05)' }}>
          {selectedAssessment && (
            <Box>
              ...
              {(selectedAssessment.type === 'vulnerability' || selectedAssessment.type === 'code') ? (
                <Box sx={{ mt: 2 }}>
                  <Typography variant="h6" gutterBottom>
                    Vulnerability & Mitigation Details
                  </Typography>
                  {((selectedAssessment.type === 'vulnerability' && selectedAssessment.data.details.length === 0) ||
                    (selectedAssessment.type === 'code' && selectedAssessment.data.vulnerabilities.length === 0)) ? (
                    ...
                    <Grid container spacing={2}>
                      {(selectedAssessment.type === 'vulnerability'
                        ...
                        <Grid item xs={12} sm={6} md={4} key={index}>
                          <StyledPaper sx={{ bgcolor: '#424242', color: '#fff', p: 2, mb: 1 }}>
        ...
        {assessments.map((assessment) => {
          const vulnerabilities =
            assessment.type === 'code'
              ...
              ? assessment.data.details || []
              : [];
          const count = (assessment.type === 'deployed' || assessment.type === 'dataset') ? 'N/A' : vulnerabilities.length;
          return (
            <React.Fragment key={assessment._id}>
              <TableRow sx={{ transition: 'background 0.3s', '&:hover': { background: 'rgba(255,255,255,0.1)' } }}>
                <StyledTableCell>{assessment.type}</StyledTableCell>
                <StyledTableCell>{new Date(assessment.createdAt).toLocaleString()}</StyledTableCell>
                <StyledTableCell>{count}</StyledTableCell>
                <StyledTableCell>
                  {vulnerabilities.length > 0 ? (
                    <Button variant="contained" sx={{ bgcolor: '#00bcd4', color: '#fff' }} onClick={() => toggleExpand(assessment._id)}>
                ...
                <StyledTableCell>
                  <Button variant="contained" sx={{ bgcolor: '#00bcd4', color: '#fff', mr: 1 }} onClick={() => handleViewDetails(assessment)}>
                    View Details
                  </Button>
                  <Button variant="contained" sx={{ bgcolor: '#4caf50', color: '#fff', mr: 1 }} onClick={() => handleDownloadPDF(assessment)}>
                    Download PDF
                  </Button>
                  <Button variant="outlined" sx={{ borderColor: '#f44336', color: '#f44336' }} onClick={() => handleDelete(assessment._id)}>
                    Delete
                  </Button>
                </StyledTableCell>
              </TableRow>
                  ...
                  <Collapse in={expandedId === assessment._id} timeout="auto" unmountOnExit>
                    <Box sx={{ m: 2, bgcolor: 'rgba(0,188,212,0.1)', p: 2, borderRadius: '8px' }}>
                      ...
                      {vulnerabilities.map((vuln, index) => (
                        <StyledPaper key={index} sx={{ bgcolor: 'rgba(255,255,255,0.1)', p: 1, mb: 1 }}>
                          <Typography variant="body2">
